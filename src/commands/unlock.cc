...
namespace commands::unlock {
    dpp::task<void> execute(const dpp::slashcommand_t& event) {
        ...
        if (overwrite.allow & dpp::p_connect) {
            co_await event.co_reply(dpp::message("Your apartment is already unlocked.").set_flags(dpp::m_ephemeral));
            co_return;
        }

        channel.remove_permission_overwrite(event.command.guild_id, dpp::ot_role);
        dpp::confirmation_callback_t update_callback = co_await event.owner->co_channel_edit(channel);
        if (update_callback.is_error()) {
            std::cerr << "Failed to unlock the apartment: " << update_callback.get_error().message << "\n";
            co_await event.co_reply(dpp::message("Failed to unlock the apartment.").set_flags(dpp::m_ephemeral));
            co_return;
        }

        co_await event.co_reply(dpp::message("Your apartment has been unlocked.").set_flags(dpp::m_ephemeral));
    }
}
